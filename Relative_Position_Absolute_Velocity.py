...
class relative_position_absolute_velocity_model:
    ...
    def initialize(self, 
        ...
        if num_instances != "all":
            ...
            all_vel_X = VXA.T[perm[:num_instances]]
            all_vel_Y = VYA.T[perm[:num_instances]]
        ...
        X_curr, Y_curr = self.transform_to_relative(all_Xpos, all_Ypos, all_vel_X, all_vel_Y, ivel)
    ...
    def transform_to_relative(self, X_pos, Y_pos, all_vel_X, all_vel_Y, ivel):
        all_rel_X = np.array([])
        all_rel_Y = np.array([])
        labels = np.array([])
        vels_X = np.array([])
        ...
        for index in range(X_pos.shape[1]):
            ...
            vels_X = np.append(vels_X, all_vel_X.T[index])
            vels_Y = np.append(vels_Y, all_vel_Y.T[index])
        vels_X = np.reshape(vels_X, [all_rel_X.shape[0], 1])
        vels_Y = np.reshape(vels_Y, [all_rel_Y.shape[0], 1])
        all_rel_pos = np.concatenate([all_rel_X, all_rel_Y, vels_X, vels_Y], axis = 1)
